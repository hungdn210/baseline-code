...
class Spatial_Attention_layer(nn.Module):
    ...
    def __init__(self, DEVICE, in_channels, num_of_vertices, num_of_timesteps):
        ...
        self.W1 = nn.Parameter(torch.FloatTensor(in_channels).to(DEVICE))  # Shape (F_in,)
        self.num_of_timesteps = num_of_timesteps
        self.W2 = None  # Initialized later
    ...
    def forward(self, x):
        ...
        :return: (B,N,N)
        '''
        if self.W2 is None or self.W2.shape[0] != x.shape[-1]:
            self.W2 = nn.Parameter(torch.FloatTensor(x.shape[-1], x.shape[-1]).to(x.device))
            nn.init.xavier_uniform_(self.W2)
        lhs = torch.einsum("bnft,f->bnt", x, self.W1)  # (B, N, T)
...
class Temporal_Attention_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        lhs = torch.matmul(torch.matmul(x.permute(0, 3, 2, 1), U1), U2)
        rhs = torch.matmul(U3, x)  # (B, N, T)

        product = torch.matmul(lhs, rhs)  # (B,T,T)

        be = torch.zeros((1, T, T), device=x.device)
        Ve = torch.eye(T, device=x.device)

        E = torch.matmul(Ve, torch.sigmoid(product + be))  # (B,T,T)
...
class ASTGCN_submodule(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.adaptive_avg_pool2d(x, (None, 1))  # (B, C, N, 1)
        output = self.final_conv(x)  # (B, T_out, N, 1)
        output = output.squeeze(-1).permute(0, 2, 1)  # (B, N, T_out)
